...
namespace ASTCEnc
{
	public static class ColorQuantize
	{
		...
		*/
		static int quant_color_clamp(QuantMethod quant_level, int value) 
		{
			value = ASTCMath.clamp(value, 0, 255);
			return Quantization.color_quant_tables[quant_level - QUANT_6, value];
		}
		...
		*         scrambles the values slightly to make hardware implementation easier.
		*/
		static byte quant_color(QuantMethod quant_level, int value) 
		{
			return Quantization.color_quant_tables[quant_level - QUANT_6, value];
		}
		...
		*         scrambles the values slightly to make hardware implementation easier.
		*/
		static byte unquant_color(QuantMethod quant_level, int value) 
		{
			return Quantization.color_unquant_tables[quant_level - QUANT_6, value];
		}
		...
		*/
		static void quantize_rgb(
			...
			do
			{
				ri0 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(r0 + rgb0_addon));
				gi0 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(g0 + rgb0_addon));
				bi0 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(b0 + rgb0_addon));
				ri1 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(r1 + rgb1_addon));
				gi1 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(g1 + rgb1_addon));
				bi1 = quant_color_clamp(quant_level, ASTCMath.flt2int_rd(b1 + rgb1_addon));
		...
		*/
		...
		) 
		{
			...
			output[6] = quant_color(quant_level, ASTCMath.flt2int_rtn(a0));
			output[7] = quant_color(quant_level, ASTCMath.flt2int_rtn(a1));
		...
		*/
		static bool try_quantize_rgb_blue_contract(
			...
			int ri0 = quant_color(quant_level, ASTCMath.flt2int_rtn(r0));
			int gi0 = quant_color(quant_level, ASTCMath.flt2int_rtn(g0));
			int bi0 = quant_color(quant_level, ASTCMath.flt2int_rtn(b0));

			int ri1 = quant_color(quant_level, ASTCMath.flt2int_rtn(r1));
			int gi1 = quant_color(quant_level, ASTCMath.flt2int_rtn(g1));
			int bi1 = quant_color(quant_level, ASTCMath.flt2int_rtn(b1));
		...
		*/
		static bool try_quantize_rgba_blue_contract(
			...
			output[6] = quant_color(quant_level, ASTCMath.flt2int_rtn(a1));
			output[7] = quant_color(quant_level, ASTCMath.flt2int_rtn(a0));
		...
		*/
		static bool try_quantize_rgb_delta(
			...
			vint4 ep0 = new vint4(r0bu, g0bu, b0bu, 0);
			vint4 ep1 = new vint4(r1du, g1du, b1du, 0);
			...
			ep0 = ep0 + ep1;
			if (any((ep0 < new vint4(0)) | (ep0 > new vint4(0xFF))))
			{
				return false;
			}
		...
		*/
		static void quantize_rgbs(
			...
			float r = ASTCMath.clamp255f(color.lane(0) * scale);
			float g = ASTCMath.clamp255f(color.lane(1) * scale);
			float b = ASTCMath.clamp255f(color.lane(2) * scale);

			int ri = quant_color(quant_level, ASTCMath.flt2int_rtn(r));
			int gi = quant_color(quant_level, ASTCMath.flt2int_rtn(g));
			int bi = quant_color(quant_level, ASTCMath.flt2int_rtn(b));
			...
			float scalea = ASTCMath.clamp1f(color.lane(3) * (oldcolorsum + 1e-10f) / (newcolorsum + 1e-10f));
		...
		*/
		static void quantize_rgbs_alpha(
			...
			output[4] = quant_color(quant_level, ASTCMath.flt2int_rtn(a0));
			output[5] = quant_color(quant_level, ASTCMath.flt2int_rtn(a1));
		...
		*/
		static void quantize_luminance(
			...
			output[0] = quant_color(quant_level, ASTCMath.flt2int_rtn(lum0));
			output[1] = quant_color(quant_level, ASTCMath.flt2int_rtn(lum1));
		}
		...
		*/
		...
		) 
		{
			...
			output[0] = quant_color(quant_level, ASTCMath.flt2int_rtn(lum0));
			output[1] = quant_color(quant_level, ASTCMath.flt2int_rtn(lum1));
			output[2] = quant_color(quant_level, ASTCMath.flt2int_rtn(a0));
			output[3] = quant_color(quant_level, ASTCMath.flt2int_rtn(a1));
		}
		...
		*/
		static void quantize_and_unquantize_retain_top_two_bits(
			QuantMethod quant_level,
			byte value,
			out byte quant_value,
			out byte unquant_value
		) 
		{
			int perform_loop;
			byte quantval;
		...
		* @param[out] unquant_value   The unquantized value after quantization.
		*/
		static void quantize_and_unquantize_retain_top_four_bits(
			QuantMethod quant_level,
			byte value,
			out byte quant_value,
			out byte unquant_value
		...
		static void quantize_hdr_rgbo(
			...
			for (int mode = 0; mode < 5; mode++)
			{
				...
				quantize_and_unquantize_retain_top_two_bits(
					quant_level, (byte)(r_lowbits), out r_quantval, out r_uquantval);

				r_intval = (r_intval & ~0x3f) | (r_uquantval & 0x3f);
				float r_fval = (float)(r_intval) * mode_rscale;
				...
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(g_lowbits), out g_quantval, out  g_uquantval);
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(b_lowbits), out b_quantval, out b_uquantval);

				g_intval = (g_intval & ~0x1f) | (g_uquantval & 0x1f);
				b_intval = (b_intval & ~0x1f) | (b_uquantval & 0x1f);

				g_fval = (float)(g_intval) * mode_rscale;
				b_fval = (float)(b_intval) * mode_rscale;
				...
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(s_lowbits), out s_quantval, out s_uquantval);
			...
			for (byte i = 0; i < 4; i++)
			{
				byte dummy;
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(encvals[i]), out output[i], out dummy);
			}
		...
		*/
		static void quantize_hdr_rgb(
			...
			case 1:  // red-green swap
				color0 = color0.swz4(1, 0, 2, 3);
				color1 = color1.swz4(1, 0, 2, 3);
				break;
			case 2:  // red-blue swap
				color0 = color0.swz4(2, 1, 0, 3);
				color1 = color1.swz4(2, 1, 0, 3);
				break;
			default:
				break;
			}
			...
			for (int mode = 7; mode >= 0; mode--)
			{
				...
				float a_fval = (float)(a_intval) * mode_rscale;
				...
				quantize_and_unquantize_retain_top_two_bits(
					quant_level, (byte)(c_lowbits), out c_quantval, out c_uquantval);

				c_intval = (c_intval & ~0x3F) | (c_uquantval & 0x3F);
				c_fval = (float)(c_intval) * mode_rscale;
				...
				quantize_and_unquantize_retain_top_two_bits(
					quant_level, (byte)(b0_lowbits), out b0_quantval, out b0_uquantval);
				quantize_and_unquantize_retain_top_two_bits(
					quant_level, (byte)(b1_lowbits), out b1_quantval, out b1_uquantval);

				b0_intval = (b0_intval & ~0x3f) | (b0_uquantval & 0x3f);
				b1_intval = (b1_intval & ~0x3f) | (b1_uquantval & 0x3f);
				b0_fval = (float)(b0_intval) * mode_rscale;
				b1_fval = (float)(b1_intval) * mode_rscale;
				...
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(d0_lowbits), out d0_quantval, out d0_uquantval);
				quantize_and_unquantize_retain_top_four_bits(
					quant_level, (byte)(d1_lowbits), out d1_quantval, out d1_uquantval);

				output[0] = (byte)(a_quantval);
				output[1] = c_quantval;
			...
			for (int i = 4; i < 6; i++)
			{
				...
				quantize_and_unquantize_retain_top_two_bits(
					quant_level, (byte)(idx), out output[i], out dummy);
			}
		...
		*/
		...
		EndpointFormats pack_color_endpoints(
			vfloat4 color0,
			vfloat4 color1,
			vfloat4 rgbs_color,
			vfloat4 rgbo_color,
			...
			Debug.Assert(QuantMethod.QUANT_6 <= quant_level && quant_level <= QuantMethod.QUANT_256);
