...
namespace ASTCEnc
{
	...
	*/
	public static class PickBestEndpointFormat
	{
		// for a given partition, compute for every (integer-component-count, quantization-level)
		// the color error.
		public static void compute_color_error_for_every_integer_count_and_quant_level(
			int encode_hdr_rgb,	// 1 = perform HDR encoding, 0 = perform LDR encoding.
			int encode_hdr_alpha,
			int partition_index,
			PartitionInfo pt,
			EncodingChoiceError eci,	// pointer to the structure for the CURRENT partition.
			Endpoints ep,
			...
			float[] baseline_quant_error = new float[21] 
			{
			...
			float ep1_min = vfloat4.hmin_rgb_s(ep1);
			ep1_min = Math.Max(ep1_min, 0.0f);

			float error_weight_rgbsum = vfloat4.hadd_rgb_s(error_weight);

			float range_upper_limit_rgb = encode_hdr_rgb == 1 ? 61440.0f : 65535.0f;
			float range_upper_limit_alpha = encode_hdr_alpha == 1 ? 61440.0f : 65535.0f;
			...
			vfloat4 offset = new vfloat4(range_upper_limit_rgb, range_upper_limit_rgb, range_upper_limit_rgb, range_upper_limit_alpha);
			ep0_range_error_high = vfloat4.max(ep0 - offset, 0.0f);
			ep1_range_error_high = vfloat4.max(ep1 - offset, 0.0f);

			ep0_range_error_low = vfloat4.min(ep0, 0.0f);
			ep1_range_error_low = vfloat4.min(ep1, 0.0f);
			...
			if (encode_hdr_rgb == 1)
			{
				...
				float[] rgbo_error_scales = new float[6] { 4.0f, 4.0f, 16.0f, 64.0f, 256.0f, 1024.0f };
				float[] rgb_error_scales = new float[9] { 64.0f, 64.0f, 16.0f, 16.0f, 4.0f, 4.0f, 1.0f, 1.0f, 384.0f };
				...
				float lum_high = vfloat4.hadd_rgb_s(ep1) * (1.0f / 3.0f);
				float lum_low = vfloat4.hadd_rgb_s(ep0) * (1.0f / 3.0f);
				...
				for (int i = 0; i < 8; i++)
				{
					...
					format_of_choice[i, 3] = encode_hdr_alpha == 1 ? EndpointFormats.FMT_HDR_RGBA : EndpointFormats.FMT_HDR_RGB_LDR_ALPHA;
					best_error[i, 2] = 1e30f;
					format_of_choice[i, 2] = EndpointFormats.FMT_HDR_RGB;
					best_error[i, 1] = 1e30f;
					format_of_choice[i, 1] = EndpointFormats.FMT_HDR_RGB_SCALE;
					best_error[i, 0] = 1e30f;
					format_of_choice[i, 0] = EndpointFormats.FMT_HDR_LUMINANCE_LARGE_RANGE;
				}
				...
				for (int i = 8; i < 21; i++)
				{
					...
					float base_quant_error = baseline_quant_error[i] * partition_size;
					...
					format_of_choice[i, 3] = encode_hdr_alpha == 1 ? FMT_HDR_RGBA : FMT_HDR_RGB_LDR_ALPHA;
			...
			else
			{
				...
				float base_quant_error_rgb = error_weight_rgbsum * partition_size;
				float base_quant_error_a = error_weight.lane(3) * partition_size;
		...
		static void one_partition_find_best_combination_for_bitcount(float[,] combined_best_error, int[,] formats_of_choice, int bits_available,
		...
		public static void two_partitions_find_best_combination_for_every_quantization_and_integer_count(
			float[,,] best_error,	// indexed by (partition, quant-level, integer-pair-count-minus-1)
			int[,,] format_of_choice,
			float[,] combined_best_error,	// indexed by (quant-level, integer-pair-count-minus-2)
			int[,,] formats_of_choice
		) {
			for (int i = 0; i < 21; i++)
			{
				for (int j = 0; j < 7; j++)
		...
		public static void two_partitions_find_best_combination_for_bitcount(
			float[,] combined_best_error,
			int[,,] formats_of_choice,
			int bits_available,
			out int best_quant_level,
			out int best_quant_level_mod,
			out int best_formats,
			out float error_of_best_combination
		) {
