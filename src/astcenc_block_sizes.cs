...
namespace ASTCEnc
{
    ...
    */
    ...
    public static class BlockSizes
    {
        ...
        */
        public static bool decode_block_mode_2d(
            uint block_mode,
            out uint x_weights,
            out uint y_weights,
            out bool is_dual_plane,
            out uint quant_mode,
            out uint weight_bits
        ) {
            uint base_quant_mode = (block_mode >> 4) & 1;
            uint H = (block_mode >> 9) & 1;
            uint D = (block_mode >> 10) & 1;
            uint A = (block_mode >> 5) & 0x3;
            ...
            uint weight_count = x_weights * y_weights * (D + 1);
        ...
        */
        static bool decode_block_mode_3d(
            ...
            uint weight_count = x_weights * y_weights * z_weights * (D + 1);
            quant_mode = (base_quant_mode - 2) + 6 * H;
            is_dual_plane = D != 0;
        ...
        */
        static void init_decimation_info_2d(
            ...
            byte max_texel_count_of_weight = 0;
            ...
            for (uint y = 0; y < y_texels; y++)
            {
                for (uint x = 0; x < x_texels; x++)
                {
                    uint texel = y * x_texels + x;

                    uint x_weight = (((1024 + x_texels / 2) / (x_texels - 1)) * x * (x_weights - 1) + 32) >> 6;
                    uint y_weight = (((1024 + y_texels / 2) / (y_texels - 1)) * y * (y_weights - 1) + 32) >> 6;
            ...
            for (uint i = 0; i < weights_per_block; i++)
            {
                uint texel_count_wt = wb.texel_count_of_weight[i];
                di.weight_texel_count[i] = (byte)(texel_count_wt);
            ...
            // Initialize array tail so we can over-fetch with SIMD later to avoid loop tails
            uint texels_per_block_simd = round_up_to_simd_multiple_vla(texels_per_block);
            for (uint i = texels_per_block; i < texels_per_block_simd; i++)
            {
            ...
            di.texel_count = (byte)(texels_per_block);
            di.weight_count = (byte)(weights_per_block);
            di.weight_x = (byte)(x_weights);
            di.weight_y = (byte)(y_weights);
            di.weight_z = 1;
        }
        ...
        */
        static void init_decimation_info_3d(
            ...
            di.weight_x = (byte)(x_weights);
            di.weight_y = (byte)(y_weights);
            di.weight_z = (byte)(z_weights);
        }

        /**
        * @brief Assign the texels to use for kmeans clustering.
        *
        * The max limit is @c Constants.BLOCK_MAX_KMEANS_TEXELS; above this a random selection is used.
        ...
        */
        static void assign_kmeans_texels(BlockSizeDescriptor bsd) 
        {
            ...
            bool[] seen = new bool[Constants.BLOCK_MAX_TEXELS];
            for (byte i = 0; i < bsd.texel_count; i++)
            {
                seen[i] = false;
            }
        ...
        */
        static void construct_dt_entry_2d(
            ...
            //assert(weight_count <= Constants.BLOCK_MAX_WEIGHTS);

            bool try_2planes = (2 * weight_count) <= Constants.BLOCK_MAX_WEIGHTS;

            DecimationInfo di = bsd.decimation_tables[index];
        ...
        */
        static void construct_block_size_descriptor_2d(
            ...
            bsd.zdim = 1;
            bsd.texel_count = (byte)(x_texels * y_texels);

            for (uint i = 0; i < MAX_DMI; i++)
            {
            ...
            uint packed_bm_idx = 0;
            uint packed_dm_idx = 0;

            // Trackers
            uint[] bm_counts = new uint[4];
            uint[] dm_counts = new uint[4];

            // Clear the list to a known-bad value
            for (uint i = 0; i < Constants.WEIGHTS_MAX_BLOCK_MODES; i++)
            {
                bsd.block_mode_packed_index[i] = Constants.BLOCK_BAD_BLOCK_MODE;
            }
            ...
            for (uint j = 0; j < limit; j ++)
            {
                for (uint i = 0; i < Constants.WEIGHTS_MAX_BLOCK_MODES; i++)
                {
                    ...
                    if (bsd.block_mode_packed_index[i] != Constants.BLOCK_BAD_BLOCK_MODE)
                    {
                        continue;
                    }
        ...
        */
        static void construct_block_size_descriptor_3d(
            ...
            bsd.xdim = (byte)(x_texels);
            bsd.ydim = (byte)(y_texels);
            bsd.zdim = (byte)(z_texels);
            bsd.texel_count = (byte)(x_texels * y_texels * z_texels);
            ...
            for (uint x_weights = 2; x_weights <= x_texels; x_weights++)
            {
                for (uint y_weights = 2; y_weights <= y_texels; y_weights++)
                {
                    for (uint z_weights = 2; z_weights <= z_texels; z_weights++)
                    {
                        uint weight_count = x_weights * y_weights * z_weights;
                        if (weight_count > Constants.BLOCK_MAX_WEIGHTS)
                        {
                            continue;
                        }
                        ...
                        if ((2 * weight_count) > Constants.BLOCK_MAX_WEIGHTS)
                        {
                            maxprec_2planes = -1;
                        }
