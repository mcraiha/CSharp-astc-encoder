...
namespace ASTCEnc
{
	public static class BlockSizes2
	{
		// return 0 on invalid mode, 1 on valid mode.
		public static int decode_block_mode_2d(int blockmode, ref int Nval, ref int Mval, ref int dual_weight_plane, ref int quant_mode) 
		{
			...
			int weightbits = IntegerSequence.get_ise_sequence_bitcount(weight_count, (QuantMethod)qmode);
			if (weight_count > Constants.MAX_WEIGHTS_PER_BLOCK ||
				weightbits < Constants.MIN_WEIGHT_BITS_PER_BLOCK ||
				weightbits > Constants.MAX_WEIGHT_BITS_PER_BLOCK)
			{
				return 0;
			}
		...
		public static int decode_block_mode_3d(int blockmode, ref int Nval, ref int Mval, ref int Qval, ref int dual_weight_plane, ref int quant_mode) 
		{
			int base_quant_mode = (blockmode >> 4) & 1;
			int H = (blockmode >> 9) & 1;
			int D = (blockmode >> 10) & 1;
		...
		*/
		public static void assign_kmeans_texels(BlockSizeDescriptor bsd) 
		{
			// Use all texels for kmeans on a small block
			if (bsd.texel_count <= Constants.MAX_KMEANS_TEXELS)
			{
			...
			bool[] seen = new bool[Constants.MAX_TEXELS_PER_BLOCK];
			for (int i = 0; i < bsd.texel_count; i++)
			{
				seen[i] = false;
			}
			...
			while (arr_elements_set < Constants.MAX_KMEANS_TEXELS)
			{
				uint idx = (uint)astc::rand(rng_state);
				idx %= bsd.texel_count;
				if (!seen[idx])
				{
			...
			bsd.kmeans_texel_count = Constants.MAX_KMEANS_TEXELS;
		}
		...
		*/
		static int construct_dt_entry_2d(int x_dim, int y_dim, int x_weights, int y_weights, BlockSizeDescriptor bsd) 
		{
			int dm_index = bsd.decimation_mode_count;
			int weight_count = x_weights * y_weights;
			Debug.Assert(weight_count <= Constants.MAX_WEIGHTS_PER_BLOCK);

			bool try_2planes = (2 * weight_count) <= Constants.MAX_WEIGHTS_PER_BLOCK;

			DecimationTable dt = new DecimationTable(unused: true);
			...
			for (int i = 0; i < 12; i++)
			{
				int bits_1plane = get_ise_sequence_bitcount(weight_count, (QuantMethod)i);
				if (bits_1plane >= Constants.MIN_WEIGHT_BITS_PER_BLOCK && bits_1plane <= Constants.MAX_WEIGHT_BITS_PER_BLOCK)
				{
					maxprec_1plane = i;
				}
				...
				if (try_2planes)
				{
					int bits_2planes = get_ise_sequence_bitcount(2 * weight_count, (QuantMethod)i);
					if (bits_2planes >= Constants.MIN_WEIGHT_BITS_PER_BLOCK && bits_2planes <= Constants.MAX_WEIGHT_BITS_PER_BLOCK)
					{
						maxprec_2planes = i;
					}
				}
			...
			Debug.Assert(maxprec_1plane >= 0 || maxprec_2planes >= 0);
		...
		*/
		public static void construct_block_size_descriptor_2d(int x_dim, int y_dim, bool can_omit_modes, float mode_cutoff, BlockSizeDescriptor bsd) 
		{
			// Store a remap table for storing packed decimation modes.
			// Indexing uses [Y * 16 + X] and max block size for each axis is 12.
			const int MAX_DMI = 12 * 16 + 12;
			...
			int packed_idx = 0;
			for (int i = 0; i < Constants.MAX_WEIGHT_MODES; i++)
			{
				int x_weights, y_weights;
				int is_dual_plane;
				int quant_mode;

				bool valid = decode_block_mode_2d(i, ref x_weights, ref y_weights, ref is_dual_plane, ref quant_mode);
			...
			for (int i = bsd.decimation_mode_count; i < Constants.MAX_DECIMATION_MODES; i++)
			{
		...
		public static void construct_block_size_descriptor_3d(int xdim, int ydim, int zdim, BlockSizeDescriptor bsd) 
		{
			int[] decimation_mode_index = new int[512];	// for each of the 512 entries in the decim_table_array, its index
			...
			for (int x_weights = 2; x_weights <= xdim; x_weights++)
			{
				for (int y_weights = 2; y_weights <= ydim; y_weights++)
				{
					for (int z_weights = 2; z_weights <= zdim; z_weights++)
					{
						...
						if ((2 * weight_count) > Constants.MAX_WEIGHTS_PER_BLOCK)
						{
							maxprec_2planes = -1;
						}
			...
			for (int i = decimation_mode_count; i < Constants.MAX_DECIMATION_MODES; i++)
			{
				bsd.decimation_modes[i].maxprec_1plane = -1;
				bsd.decimation_modes[i].maxprec_2planes = -1;
				bsd.decimation_modes[i].percentile_hit = false;
			...
			for (int i = 0; i < Constants.MAX_WEIGHT_MODES; i++)
			{
				...
				if (decode_block_mode_3d(i, ref x_weights, ref y_weights, ref z_weights, ref is_dual_plane, ref quant_mode))
				{
		...
		public static void init_block_size_descriptor(int xdim, int ydim, int zdim, bool can_omit_modes, float mode_cutoff, BlockSizeDescriptor bsd) 
		{
			if (zdim > 1)
			{
		...
		public static void term_block_size_descriptor(BlockSizeDescriptor bsd)
		{
