...
namespace ASTCEnc
{
    public static class FindBestPartioning
    {
        ...
        */
        static void kmeans_init(ImageBlock blk, uint texel_count, uint partition_count,vfloat4[] cluster_centers) 
        {
            //promise(texel_count > 0);
            //promise(partition_count > 0);

            uint clusters_selected = 0;
            float[] distances = new float[Constants.BLOCK_MAX_TEXELS];

            // Pick a random sample as first cluster center; 145897 from random.org
            uint sample = 145897 % texel_count;
            ...
            while (true)
            {
                ...
                sample = ASTCMath.min(sample, texel_count - 1);
                ...
                for (uint i = 0; i < texel_count; i++)
                {
                    ...
                    vfloat4 diff = color - center_color;
                    float distance = vfloat4.dot_s(diff * diff, blk.channel_weight);
                    distance = ASTCMath.min(distance, distances[i]);
                    distance_sum += distance;
                    distances[i] = distance;
                }
            }
        ...
        */
        static void kmeans_assign(ImageBlock blk, uint texel_count, uint partition_count, vfloat4[] cluster_centers, byte[] partition_of_texel) 
        {
            //promise(texel_count > 0);
            //promise(partition_count > 0);

            byte[] partition_texel_count = new byte[Constants.BLOCK_MAX_PARTITIONS];
            ...
            for (uint i = 0; i < texel_count; i++)
            {
                ...
                partition_of_texel[i] = (byte)(best_partition);
        ...
        */
        static void kmeans_update(
            ...
            byte[] partition_of_texel
        ) {
            //promise(texel_count > 0);
            //promise(partition_count > 0);

            vfloat4[] color_sum = new vfloat4[Constants.BLOCK_MAX_PARTITIONS] {
                vfloat4.zero(),
                vfloat4.zero(),
                vfloat4.zero(),
                vfloat4.zero()
            ...
            for (uint i = 0; i < texel_count; i++)
            {
                byte partition = partition_of_texel[i];
            ...
            for (uint i = 0; i < partition_count; i++)
            {
                float scale = 1.0f / (float)(partition_texel_count[i]);
        ...
        */
        static uint partition_mismatch2(ulong[] a,ulong[] b) 
        {
            int v1 = ASTCMath.popcount(a[0] ^ b[0]) + ASTCMath.popcount(a[1] ^ b[1]);
            int v2 = ASTCMath.popcount(a[0] ^ b[1]) + ASTCMath.popcount(a[1] ^ b[0]);
            return ASTCMath.min(v1, v2);
        }
        ...
        * @return    The number of bit mismatches.
        */
        static uint partition_mismatch3(ulong[] a, ulong[] b) 
        {
            int p00 = ASTCMath.popcount(a[0] ^ b[0]);
            int p01 = ASTCMath.popcount(a[0] ^ b[1]);
            int p02 = ASTCMath.popcount(a[0] ^ b[2]);

            int p10 = ASTCMath.popcount(a[1] ^ b[0]);
            int p11 = ASTCMath.popcount(a[1] ^ b[1]);
            int p12 = ASTCMath.popcount(a[1] ^ b[2]);

            int p20 = ASTCMath.popcount(a[2] ^ b[0]);
            int p21 = ASTCMath.popcount(a[2] ^ b[1]);
            int p22 = ASTCMath.popcount(a[2] ^ b[2]);

            int s0 = p11 + p22;
            int s1 = p12 + p21;
            int v0 = ASTCMath.min(s0, s1) + p00;

            int s2 = p10 + p22;
            int s3 = p12 + p20;
            int v1 = ASTCMath.min(s2, s3) + p01;

            int s4 = p10 + p21;
            int s5 = p11 + p20;
            int v2 = ASTCMath.min(s4, s5) + p02;

            return ASTCMath.min(v0, v1, v2);
        }
        ...
        * @return    The number of bit mismatches.
        */
        static uint partition_mismatch4(ulong[] a, ulong[] b) 
        {
            int p00 = ASTCMath.popcount(a[0] ^ b[0]);
            int p01 = ASTCMath.popcount(a[0] ^ b[1]);
            int p02 = ASTCMath.popcount(a[0] ^ b[2]);
            int p03 = ASTCMath.popcount(a[0] ^ b[3]);

            int p10 = ASTCMath.popcount(a[1] ^ b[0]);
            int p11 = ASTCMath.popcount(a[1] ^ b[1]);
            int p12 = ASTCMath.popcount(a[1] ^ b[2]);
            int p13 = ASTCMath.popcount(a[1] ^ b[3]);

            int p20 = ASTCMath.popcount(a[2] ^ b[0]);
            int p21 = ASTCMath.popcount(a[2] ^ b[1]);
            int p22 = ASTCMath.popcount(a[2] ^ b[2]);
            int p23 = ASTCMath.popcount(a[2] ^ b[3]);

            int p30 = ASTCMath.popcount(a[3] ^ b[0]);
            int p31 = ASTCMath.popcount(a[3] ^ b[1]);
            int p32 = ASTCMath.popcount(a[3] ^ b[2]);
            int p33 = ASTCMath.popcount(a[3] ^ b[3]);

            int mx23 = ASTCMath.min(p22 + p33, p23 + p32);
            int mx13 = ASTCMath.min(p21 + p33, p23 + p31);
            int mx12 = ASTCMath.min(p21 + p32, p22 + p31);
            int mx03 = ASTCMath.min(p20 + p33, p23 + p30);
            int mx02 = ASTCMath.min(p20 + p32, p22 + p30);
            int mx01 = ASTCMath.min(p21 + p30, p20 + p31);

            int v0 = p00 + ASTCMath.min(p11 + mx23, p12 + mx13, p13 + mx12);
            int v1 = p01 + ASTCMath.min(p10 + mx23, p12 + mx03, p13 + mx02);
            int v2 = p02 + ASTCMath.min(p11 + mx03, p10 + mx13, p13 + mx01);
            int v3 = p03 + ASTCMath.min(p11 + mx02, p12 + mx01, p10 + mx12);

            return ASTCMath.min(v0, v1, v2, v3);
        }

        //using mismatch_dispatch = uint (*)(const ulong*, const ulong*);
        ...
        */
        static void count_partition_mismatch_bits(BlockSizeDescriptor bsd, uint partition_count, ulong[] bitmaps, uint[] mismatch_counts) 
        {
            uint active_count = bsd.partitioning_count_selected[partition_count - 1];
        ...
        */
        static uint get_partition_ordering_by_mismatch_bits(uint partitioning_count, uint[] mismatch_count, uint[] partition_ordering
        ) {
            uint[] mscount = new uint[256];

            // Create the histogram of mismatch counts
            for (uint i = 0; i < partitioning_count; i++)
            {
                mscount[mismatch_count[i]]++;
            }

            uint active_count = partitioning_count - mscount[255];
            ...
            for (uint i = 0; i < partitioning_count; i++)
            {
                uint idx = mscount[mismatch_count[i]]++;
        ...
        */
        static uint compute_kmeans_partition_ordering(BlockSizeDescriptor bsd, ImageBlock blk, uint partition_count, uint[] partition_ordering
        ) {
            vfloat4[] cluster_centers = new vfloat4[Constants.BLOCK_MAX_PARTITIONS];
            byte[] texel_partitions = new byte[Constants.BLOCK_MAX_TEXELS];
            ...
            ulong[] bitmaps = new ulong[Constants.BLOCK_MAX_PARTITIONS];
            ...
            for (uint i = 0; i < texels_to_process; i++)
            {
                ...
                bitmaps[texel_partitions[idx]] |= 1 << i;
            }

            // Count the mismatch between the block and the format's partition tables
            uint[] mismatch_counts = new uint[Constants.BLOCK_MAX_PARTITIONINGS];
        ...
        */
        ...
        static uint find_best_partition_candidates(BlockSizeDescriptor bsd, ImageBlock blk, uint partition_count, uint partition_search_limit, uint best_partitions, uint requested_candidates) 
        {
            ...
            uint texels_per_block = bsd.texel_count;
            ...
            uint[] partition_sequence = new uint[Constants.BLOCK_MAX_PARTITIONINGS];
            uint sequence_len = compute_kmeans_partition_ordering(bsd, blk, partition_count, partition_sequence);
            partition_search_limit = ASTCMath.min(partition_search_limit, sequence_len);
            requested_candidates = ASTCMath.min(partition_search_limit, requested_candidates);

            bool uses_alpha = !blk.is_constant_channel(3);

            // Partitioning errors assuming uncorrelated-chrominance endpoints
            float[] uncor_best_errors = new float[Constants.TUNE_MAX_PARTITIIONING_CANDIDATES];
            uint[] uncor_best_partitions = new uint[Constants.TUNE_MAX_PARTITIIONING_CANDIDATES];

            // Partitioning errors assuming same-chrominance endpoints
            float[] samec_best_errors = new float[Constants.TUNE_MAX_PARTITIIONING_CANDIDATES];
            uint[] samec_best_partitions = new uint[Constants.TUNE_MAX_PARTITIIONING_CANDIDATES];

            for (uint i = 0; i < requested_candidates; i++)
            {
                uncor_best_errors[i] = Constants.ERROR_CALC_DEFAULT;
                samec_best_errors[i] = Constants.ERROR_CALC_DEFAULT;
            }

            if (uses_alpha)
            {
                for (uint i = 0; i < partition_search_limit; i++)
                {
                    uint partition = partition_sequence[i];
                    PartitionInfo pi = bsd.get_raw_partition_info(partition_count, partition);
                    ...
                    Line4[] uncor_lines = new Line4[Constants.BLOCK_MAX_PARTITIONS];
                    Line4[] samec_lines = new Line4[Constants.BLOCK_MAX_PARTITIONS];

                    ProcessedLine4[] uncor_plines = new ProcessedLine4[Constants.BLOCK_MAX_PARTITIONS];
                    ProcessedLine4[] samec_plines = new ProcessedLine4[Constants.BLOCK_MAX_PARTITIONS];

                    float[] uncor_line_lens = new float[Constants.BLOCK_MAX_PARTITIONS];
                    float[] samec_line_lens = new float[Constants.BLOCK_MAX_PARTITIONS];

                    for (uint j = 0; j < partition_count; j++)
                    {
                        PartitionMetrics pm = pms[j];
                    ...
                    for (uint j = 0; j < partition_count; j++)
                    {
                        ...
                        vfloat4 error_weights = new vfloat4(tpp * weight_imprecision_estim);

                        vfloat4 uncor_vector = uncor_lines[j].b * uncor_line_lens[j];
                        vfloat4 samec_vector = samec_lines[j].b * samec_line_lens[j];

                        uncor_error += vfloat4.dot_s(uncor_vector * uncor_vector, error_weights);
                        samec_error += vfloat4.dot_s(samec_vector * samec_vector, error_weights);
                    }
            ...
            else
            {
                for (uint i = 0; i < partition_search_limit; i++)
                {
                    ...
                    PartitionLines3[] plines = new PartitionLines3[Constants.BLOCK_MAX_PARTITIONS];

                    for (uint j = 0; j < partition_count; j++)
                    {
                        PartitionMetrics pm = pms[j];
                        PartitionLines3 pl = plines[j];
                    ...
                    for (uint j = 0; j < partition_count; j++)
                    {
                        ...
                        uncor_error += vfloat4.dot3_s(uncor_vector * uncor_vector, error_weights);
                        samec_error += vfloat4.dot3_s(samec_vector * samec_vector, error_weights);
                    }
            ...
            uint[] interleave = new uint[2 * Constants.TUNE_MAX_PARTITIIONING_CANDIDATES];
            for (uint i = 0; i < requested_candidates; i++)
            {
                if (best_is_uncor)
                {
            ...
            ulong[] bitmasks = new ulong[1024/64];
            uint emitted = 0;

            // Deduplicate the first "requested" entries
            for (uint i = 0; i < requested_candidates * 2;  i++)
            {
                uint partition = interleave[i];

                uint word = partition / 64;
                uint bit = partition % 64;

                bool written = bitmasks[word] & (1 << bit);
