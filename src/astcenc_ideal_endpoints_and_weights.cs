...
namespace ASTCEnc
{
    public static class IdealEndpointsAndWeights
    {
        ...
        */
        static vfloat bilinear_infill_vla(DecimationInfo di, float[] weights, uint index) {
            ...
            vfloat weight_val0 = vfloat4.gatherf(weights, weight_idx0);
            vfloat weight_val1 = vfloat4.gatherf(weights, weight_idx1);
            vfloat weight_val2 = vfloat4.gatherf(weights, weight_idx2);
            vfloat weight_val3 = vfloat4.gatherf(weights, weight_idx3);
        ...
        */
        static void compute_ideal_colors_and_weights_1_comp(
            ...
            for (uint i = 0; i < partition_count; i++)
            {
                ...
                uint partition_texel_count = pi.partition_texel_count[i];
                for (uint j = 0; j < partition_texel_count; j++)
                {
                    uint tix = pi.texels_of_partition[i][j];
                    float value = data_vr[tix];
                    lowvalue = ASTCMath.min(value, lowvalue);
                    highvalue = ASTCMath.max(value, highvalue);
                }
            ...
            uint texel_count_simd = round_up_to_simd_multiple_vla(texel_count);
            for (uint i = texel_count; i < texel_count_simd; i++)
            {
                ei.weights[i] = 0.0f;
        ...
        */
        static void compute_ideal_colors_and_weights_2_comp(
            ...
            PartitionMetrics[] pms = new PartitionMetrics[Constants.BLOCK_MAX_PARTITIONS];
            ...
            {
                Debug.Assert(component1 == 1 && component2 == 2);
            ...
            for (uint i = 0; i < partition_count; i++)
            {
                ...
                for (uint j = 0; j < partition_texel_count; j++)
                {
                    ...
                    lowparam = ASTCMath.min(param, lowparam);
                    highparam = ASTCMath.max(param, highparam);
                }
        ...
        */
        static void compute_ideal_colors_and_weights_3_comp(ImageBlock blk, PartitionInfo pi, EndpointsAndWeights ei, uint omitted_component) 
        {
            ...
            {
                Debug.Assert(omitted_component == 3);
        ...
        */
        static void compute_ideal_colors_and_weights_4_comp(
            ...
            float error_weight = vfloat4.hadd_s(blk.channel_weight) / 4.0f;
        ...
        /* See header for documentation. */
        static float compute_error_of_weight_set_1plane(
            EndpointsAndWeights eai,
            DecimationInfo di,
        ...
        static void compute_ideal_weights_for_decimation(EndpointsAndWeights ei, DecimationInfo di, float[] dec_weight_ideal_value) 
        {
            ...
            uint prev_weight_count_simd = round_down_to_simd_multiple_vla(weight_count - 1);
            ...
            {
                uint texel_count_simd = round_up_to_simd_multiple_vla(texel_count);
                for (uint i = 0; i < texel_count_simd; i += Constants.ASTCENC_SIMD_WIDTH)
                {
                    vfloat weight = new vfloat(ei.weights + i);
            ...
            float[] infilled_weights = new float[Constants.BLOCK_MAX_TEXELS];

            // Compute an initial average for each decimated weight
            bool constant_wes = ei.is_constant_weight_error_scale;
            vfloat weight_error_scale = new vfloat(ei.weight_error_scale[0]);
            ...
            for (uint i = 0; i < weight_count; i += Constants.ASTCENC_SIMD_WIDTH)
            {
                ...
                for (uint j = 0; j < max_texel_count; j++)
                {
                    ...
                    if (!constant_wes)
                    {
                        weight_error_scale = vfloat4.gatherf(ei.weight_error_scale, texel);
                    }

                    vfloat contrib_weight = weight * weight_error_scale;

                    weight_weight += contrib_weight;
                    initial_weight += vfloat4.gatherf(ei.weights, texel) * contrib_weight;
                }
            ...
            for (uint i = 0; i < weight_count; i += Constants.ASTCENC_SIMD_WIDTH)
            {
                ...
                // Accumulate error weighting of all the texels using this weight
                // Start with a small value to avoid div-by-zero later
                vfloat error_change0 = new vfloat(1e-10f);
                ...
                for (uint j = 0; j < max_texel_count; j++)
                {
                    ...
                    vfloat old_weight = vfloat4.gatherf(infilled_weights, texel);
                    vfloat ideal_weight = vfloat4.gatherf(ei.weights, texel);
        ...
        void compute_quantized_weights_for_decimation(DecimationInfo di, float low_bound, float high_bound, float[] dec_weight_ideal_value, float[] weight_set_out, byte[] quantized_weight_set, QuantMethod quant_level) 
        {
            ...
            vint steps_m1 = new vint(get_quant_level(quant_level) - 1);
            ...
            vfloat scalev = new vfloat(scale);
            vfloat scaled_low_boundv = new vfloat(scaled_low_bound);
            vfloat quant_level_m1v = new vfloat(quant_level_m1);
            vfloat rscalev = new vfloat(rscale);
            vfloat low_boundv = new vfloat(low_bound);
        ...
        */
        static vfloat4 compute_rgbo_vector(
        ...
        void recompute_ideal_colors_1plane(
            ...
            PartitionInfo pi,
            DecimationInfo di,
            byte[] dec_weights_uquant,
            Endpoints ep,
            vfloat4[] rgbs_vectors,
            vfloat4[] rgbo_vectors
        ) {
            uint weight_count = di.weight_count;
            uint total_texel_count = blk.texel_count;
            uint partition_count = pi.partition_count;
            ...
            float[] dec_weight = new float[Constants.BLOCK_MAX_WEIGHTS];
            for (uint i = 0; i < weight_count; i += Constants.ASTCENC_SIMD_WIDTH)
            {
                vint unquant_value = new vint(dec_weights_uquant + i);
                vfloat unquant_valuef = int_to_float(unquant_value) * vfloat(1.0f / 64.0f);
                storea(unquant_valuef, dec_weight + i);
            }

            float[] undec_weight = new float[Constants.BLOCK_MAX_TEXELS];
            ...
            {
                for (uint i = 0; i < total_texel_count; i += Constants.ASTCENC_SIMD_WIDTH)
                {
            ...
            vfloat4 rgba_sum(blk.data_mean * (float)(blk.texel_count));

            for (uint i = 0; i < partition_count; i++)
            {
                uint texel_count = pi.partition_texel_count[i];
                byte[] texel_indexes = pi.texels_of_partition[i];

                // Only compute a partition mean if more than one partition
                if (partition_count > 1)
                {
                    rgba_sum = new vfloat4(1e-17f);
                    //promise(texel_count > 0);
                    for (uint j = 0; j < texel_count; j++)
                    {
                        uint tix = texel_indexes[j];
                        rgba_sum += blk.texel(tix);
                    }
                }

                rgba_sum = rgba_sum * blk.channel_weight;
                vfloat4 rgba_weight_sum = max(blk.channel_weight * (float)(texel_count), 1e-17f);
                ...
                for (uint j = 0; j < texel_count; j++)
                {
                    ...
                    wmin1 = ASTCMath.min(idx0, wmin1);
                    wmax1 = ASTCMath.max(idx0, wmax1);

                    float scale = dot3_s(scale_dir, rgba);
                    scale_min = ASTCMath.min(scale, scale_min);
                    scale_max = ASTCMath.max(scale, scale_max);
                ...
                float scalediv = scale_min / ASTCMath.max(scale_max, 1e-10f);
        ...
        void recompute_ideal_colors_2planes(
            ...
            DecimationInfo di,
            byte[] dec_weights_uquant_plane1,
            byte[] dec_weights_uquant_plane2,
            Endpoints ep,
            vfloat4 rgbs_vector,
            vfloat4 rgbo_vector,
            ...
            float[] dec_weight_plane1 = new float[Constants.BLOCK_MAX_WEIGHTS_2PLANE];
            float[] dec_weight_plane2 = new float[Constants.BLOCK_MAX_WEIGHTS_2PLANE];

            Debug.Assert(weight_count <= Constants.BLOCK_MAX_WEIGHTS_2PLANE);

            for (uint i = 0; i < weight_count; i += Constants.ASTCENC_SIMD_WIDTH)
            {
                vint unquant_value1 = new vint(dec_weights_uquant_plane1 + i);
                vfloat unquant_value1f = int_to_float(unquant_value1) * vfloat(1.0f / 64.0f);
                storea(unquant_value1f, dec_weight_plane1 + i);

                vint unquant_value2 = new vint(dec_weights_uquant_plane2 + i);
                vfloat unquant_value2f = int_to_float(unquant_value2) * vfloat(1.0f / 64.0f);
                storea(unquant_value2f, dec_weight_plane2 + i);
            }

            float[] undec_weight_plane1 = new float[Constants.BLOCK_MAX_TEXELS];
            float[] undec_weight_plane2 = new float[Constants.BLOCK_MAX_TEXELS];
            ...
            for (uint j = 0; j < texel_count; j++)
            {
                ...
                wmin2 = ASTCMath.min(idx1, wmin2);
                wmax2 = ASTCMath.max(idx1, wmax2);
